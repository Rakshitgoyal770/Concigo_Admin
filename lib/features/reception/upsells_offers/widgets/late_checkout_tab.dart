  ...
  void _confirmResetOfferClaims(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: Row(
          children: [
            const Icon(Icons.restart_alt_rounded, color: AppColors.purple, size: 22),
            const SizedBox(width: 8),
            Text('Reset Offer Claims', style: AppTypography.titleMedium),
          ],
        ),
        content: Text(
          'Are you sure you want to reset the claimed passes quota for "$category"?\n\nThis starts a fresh quota batch (0 / ${limit != null ? limit.toString() : 'Unlimited'} claimed) so upcoming guests can purchase late check-out passes again without deleting any past guest billing records.',
