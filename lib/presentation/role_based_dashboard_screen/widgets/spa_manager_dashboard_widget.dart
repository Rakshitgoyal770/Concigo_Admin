...
class SpaManagerDashboardWidget extends StatefulWidget {
  ...
  const SpaManagerDashboardWidget({
    super.key,
  ...
  @override
  State<SpaManagerDashboardWidget> createState() =>
      _SpaManagerDashboardWidgetState();
}

class _SpaManagerDashboardWidgetState extends State<SpaManagerDashboardWidget>
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await Future.wait([
        SupabaseService.instance.fetchSpaOrders(propertyId: widget.propertyId),
        SupabaseService.instance.fetchSpaEmployees(widget.propertyId),
      ]);
      if (mounted) {
        setState(() {
          _requests = results[0];
  ...
  List<Map<String, dynamic>> get _filteredRequests {
    ...
    if (_statusFilter == 'pending') {
      return _requests.where((r) => r['status'] == 'enquired').toList();
    }
    ...
    if (_statusFilter == 'completed') {
      return _requests.where((r) => r['status'] == 'serviced').toList();
    }
    return _requests;
  }
  ...
  Widget _buildOverview() {
    final pending = _requests.where((r) => r['status'] == 'enquired').length;
    final assigned = _requests
        ...
        .length;
    final completed = _requests.where((r) => r['status'] == 'serviced').length;
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              const Icon(Icons.spa_rounded, color: Colors.white54, size: 40),
            ],
          ),
        ),
        const SizedBox(height: 16),
        ...
        if (_requests.isEmpty)
          EmptyStateWidget(
            ...
            title: 'No spa requests',
            description: 'Spa booking requests will appear here.',
          )
        else
  ...
  Widget _buildRequestsList() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Manage and assign spa bookings',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
            color: AppTheme.onSurfaceMuted,
          ),
        ...
        if (_filteredRequests.isEmpty)
          EmptyStateWidget(
            icon: Icons.spa_rounded,
            title: 'No requests',
            description: 'No spa requests match this filter.',
          )
        else
  ...
  Widget _buildAssignSection() {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          'Assign spa employees to requests',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
            color: AppTheme.onSurfaceMuted,
          ),
        ...
        if (_employees.isEmpty)
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'No spa employees found for this property.',
                    style: GoogleFonts.plusJakartaSans(
                      fontSize: 13,
                      color: AppTheme.warning,
                    ),
          ...
          EmptyStateWidget(
            ...
            title: 'All assigned',
            description: 'No unassigned spa requests.',
          )
        else
  ...
  Widget _buildBillingSection() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(
          'Create offline bills for spa services',
          style: GoogleFonts.plusJakartaSans(
            fontSize: 13,
            color: AppTheme.onSurfaceMuted,
          ),
        ...
        children: tabs.map((tab) {
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: isActive ? _spaColor : AppTheme.surface,
                borderRadius: BorderRadius.circular(100),
                border: Border.all(
                  color: isActive ? _spaColor : AppTheme.outline,
                ),
              ),
              child: Text(
                tab.$2,
  ...
  Widget _buildRequestCard(Map<String, dynamic> req) {
    final reqId = req['spa_order_id'] as String? ?? '';
    final guestName = req['guest_name'] as String? ?? 'Guest';
    final roomNumber = req['room_number'] as String? ?? '-';
    final serviceName = req['service_name'] as String? ?? 'Spa Service';
    final slotTime = req['slot_time'] as String? ?? '-';
    final mobileNo = req['mobile_no'] as String? ?? '';
    final status = req['status'] as String? ?? 'enquired';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: _spaColor.withAlpha(40), width: 1),
        boxShadow: [
          BoxShadow(
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                const Icon(Icons.spa_rounded, size: 14, color: _spaColor),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                if (status != 'serviced' && status != 'cancelled') ...[
                  const SizedBox(height: 12),
    ...
    return Row(
      children: [
        if (status == 'enquired')
          Expanded(
            child: ElevatedButton.icon(
              onPressed: () => _updateStatus(reqId, 'called_back'),
        ...
        if (status == 'called_back') ...[
          Expanded(
            child: ElevatedButton.icon(
              onPressed: () => _updateStatus(reqId, 'serviced'),
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(10),
                borderSide: const BorderSide(color: _spaColor, width: 2),
              ),
  ...
  Future<void> _assignEmployee(String reqId, String empId) async {
    try {
      ...
      await SupabaseService.instance.assignSpaOrder(reqId, empId);
